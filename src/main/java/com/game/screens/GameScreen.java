...
public class GameScreen/* implements Screen*/ {
  ...
  public GameScreen(File map, int playerNum) {
    ...
    if (centralProcessor.getPhysicalProcessorCount() == centralProcessor.getLogicalProcessorCount()) {
      threadCount = StrictMath.max(centralProcessor.getPhysicalProcessorCount() - 1, 1);
    } else {
      threadCount = StrictMath.max(centralProcessor.getLogicalProcessorCount() - 2, 1);
    }
...
//        entitiesById = new Int2ObjectOpenHashMap<>();
    ...
    System.out.println("physics threads: " + threadCount);
    load();
  ...
  private void load() {
    loadingScreen = new LoadingScreen(0/*Models.values().length * 5 + Models.getUnitAmount() * 32 + Models.getBuildingAmount() * 2*/);
...
//            System.out.println("spatial grid: " + (t2-t1)/1000000d + "detect: " + (t3-t2)/1000000d + "resolve: " + (t4-t3)/1000000d + "collisions amount: " + collisions.size());
  ...
  public void updateOnFrame() {
    long t1 = System.nanoTime();
    ReplayScreen.addTick(InputHandler.getInputs(), tickNum);
    tickNum++;
      ...
      switch (input.getInputType()) {
        case LEFT_CLICK: {
          ...
          while (clickIdx != -1) {
            ...
            float x = NumUtil.interpolate(entity.getLastX(), entity.getX(), DrawManager.getFactor());
            float y = NumUtil.interpolate(entity.getLastY(), entity.getY(), DrawManager.getFactor());
...
//        System.out.println("input: " + (t2-t1)/1000000d + "update: " + (t3-t2)/1000000d + "ms  physics: " + (t4-t3)/1000000d);
  ...
  public void drawHud() {
    DrawManager.fillRect(0, 0, 1920, 260, 0x505050FF);
    DrawManager.fillLine(260, 200, 1570, 200, 0x000000FF, 4);
    DrawManager.fillLine(0, 260, 1920, 260, 0x000000FF, 4);
    DrawManager.fillLine(260, 260, 260, 0, 0x000000FF, 4);
    DrawManager.fillLine(1570, 260, 1570, 0, 0x000000FF, 4);
...
////            }
//      for (int i = 0; i < 28; i++) {
//        DrawManager.strokeRect(290 + (counter % 14) * 90, (float) (100 - (counter / 14) * 90), 75, 75, 0x0096FFFF, 4);//float is there to supress warning
//        counter++;
//      }
//      //compressed
//    } else {
...
//        TODO: make a special y that is set here and then sorted to get rid of concurreny issues, and interpolate this y value too;
    for (Entity entity : visibleEntities) {
      entity.getSnapshot(isSnapshot1Draw).setYLerp(DrawManager.getFactor());
    }
    if (isSnapshot1Draw) {
