...
import com.game.inputHandler.Keys;
...
public class MapEditorScreen extends GameWindow implements Screen {
    ...
    private MapEditorScreen(MapEditorScreen mapEditorScreen) {
        directory = mapEditorScreen.directory;
        isRenaming = mapEditorScreen.isRenaming;
        newName = mapEditorScreen.newName;

        tileMap = (ArrayList<ArrayList<Integer>>) mapEditorScreen.tileMap.clone();
        heightMap = (ArrayList<ArrayList<Integer>>) mapEditorScreen.heightMap.clone();
        tiles = (ArrayList<Image>) mapEditorScreen.tiles.clone();
        units = new ArrayList<>();
        buildings = new ArrayList<>();
        objects = new ArrayList<>();
        tileWidth = mapEditorScreen.tileIndex;
        tileIndex = mapEditorScreen.tileIndex;
        height = mapEditorScreen.height;
        exit = mapEditorScreen.exit;
    }
    ...
    public GameWindow copy() {
        return new MapEditorScreen(this);
    }
...
//                    Object object = parser.parse(new FileReader(directory + "/map.json"));
//                    JSONObject map = (JSONObject) object;
//                    DrawUtil.drawString(button.getRectangle().getX() + button.getRectangle().getWidth() / 2, button.getRectangle().getY() + button.getRectangle().getHeight() / 2, (String) map.get("name"), 20, Fonts.DEFAULT);
//                } catch (IOException | ParseException e) {
//                    throw new RuntimeException(e);
//                }
                continue;
